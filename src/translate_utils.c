...
int translate_num(long int* output, const char* str, long int lower_bound, 
    ...
    int low = 0;
    int good = 0;
    for (len = strlen(str); low < len; low++) {
        if (!isdigit(str[low])) {
            if (low == 1) {
                if (str[low] == 120) { // 120 means "x" which is hex
                    //it is a hex so it is ok
                    good = 1;
                } else {
                    return -1;
                }
            } else if (good == 1 && (str[low] >= 65 && str[low] <= 70)) { // between "A" and "F"
                //it is still a valid hex
            } else { // it is not a hex so return -1
                return -1;
            }
        }
    }
